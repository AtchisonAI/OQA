...
namespace OQAService.Services
{
    ...
    {
        public ModelRsp<WaferInspectRecordView> QueryWaferInspectionRecordInfo(ModelRsp<WaferInspectRecordView> WaferInspectRecord)
        {
            try
            {
                //定义服务过程中使用的结构
                ...
                if (In_node.model.C_PROC_STEP == GlobalConstant.TRAN_VIEW)
                {
                    ...
                    switch (In_node.model.C_TRAN_FLAG)
                    {
                        case '1':
                            ...
                            {
                                AddCondition(PageQueryReq, GetParaName<ISPWAFDFT>(p => p.LotId), In_node.model.IN_LOTID.Trim(), LogicCondition.AndAlso,CompareType.Equal);
                            }
                            ...
                            var data = PageQuery<ISPWAFDFT>(PageQueryReq);
                            out_list.ISPWAFDFT_list = data.models;
