...
namespace OQAService.Services
{
    ...
    {
        public ModelRsp<PKGShipView> QryPKGShipSummaryInfo(ModelRsp<PKGShipView> PKGShipSummary)
        {
            try
            {
                //定义服务过程中使用的结构
                ...
                PKGShipView out_list = new PKGShipView();

                In_node.model = PKGShipSummary.model;
                ...
                if (In_node.model.C_PROC_STEP == GlobalConstant.TRAN_VIEW)
                {
                    ...
                    switch (In_node.model.C_TRAN_FLAG)
                    {
                        case '1':
                            ...
                            {
                                AddCondition(PageQueryReq, GetParaName<PKGSHPSTS>(p=>p.ShipId), In_node.model.IN_SHIP_NO.Trim(), LogicCondition.AndAlso,CompareType.Equal);
                            }
                            ...
                            Out_node.model.PKGSHP_list = data.models;
